...
- account_item*: object
  ...
  - code: string - 勘定科目コード 例: `999` (パターン: ^[0-9a-zA-Z_-]+$)
  ...
  - items: array[object] - 品目
  - partners: array[object] - 取引先
...
- company_id*: integer(int64) - 事業所ID
- base_date: string - 基準日:指定した場合、勘定科目に紐づく税区分(default_tax_code)が、基準日の税率に基づいて返ります。
...
概要 勘定科目コードをキーに、指定した勘定科目の情報を更新（存在しない場合は作成）する
